...
$(function() {
  window.addEventListener('message', function(event) {
    ...
    if (event.data.type == "openGeneral"){
      ...
      $('#saldos').html('Penge: '+event.data.cash+' Kr')
...
if (bet == null) {
  $msg.html('Du skal vælge en farve.')
} else {
var stawka = $('#stawka').val();
var saldo = $saldor.html();
if (+stawka < 100) {
  $msg.html('Minimumsindsatsen skal være 100 kr')
} else if (+stawka > max_bet) {
  $msg.html('Det maksimale beløb, du kan satse, er 50.000 kr')
} else {
  if (+stawka > +saldo) {
    $msg.html('Du har ikke nok penge nok på dig.')
  } else {
  foo.play();
  var wlasciwesaldo = +saldo - +stawka
  $saldor.html(wlasciwesaldo);
  $saldoh.html('Penge: '+wlasciwesaldo+'$');
 ...
 var  randomNumber = Math.floor(Math.random() * 36),
  ...
  setTimeout(function() {
    ...
    if (color == bet) {
      $thisResult = '<li style="color:#ffbf00;" class="previous-result color-'+ color +'"><span class="previous-number">'+ randomNumber +'</span><span class="previous-color">Du vandt </span></li>';
      $msg.html('<span style="color:#ffbf00;">Du vandt '+wygrana+'$');
      ...
      $msg.html('Du tabte.');
      $thisResult = '<li class="previous-result color-'+ color +'"><span class="previous-number">'+ randomNumber +'</span><span class="previous-color">Du tabte !</span></li>';
        losdio.play()
      console.log('Han tabte'+tolose)
    }
